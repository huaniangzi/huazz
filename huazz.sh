#!/bin/bash
ln -sf ~/huazz.sh /usr/local/bin/h
...
echo "| | |_| | |  | \| | | | | \| |__|  /__ | "
echo "                                "
echo -e "\033[96m花娘子一键脚本工具 v1.0.0 （支持Ubuntu/Debian/CentOS/Alpine系统）\033[0m"
    ...
    while true; do
      ...
      case $sub_choice in
          1)
              ...
              echo "alias $kuaijiejian='~/huanzz.sh'" >> ~/.bashrc
              source ~/.bashrc
              echo "快捷键已设置"
              ;;
...
EOF
          ...
          21)
              ...
              while true; do
                  ...
                  case $host_dns in
                      1)
                          read -p "请输入新的解析记录 格式: 110.25.5.33 huaniangzi.pro : " addhost
